...
WEBHOOK_SECRET_TOKEN=use_a_long_random_secret_for_the_telegram_header
WEBHOOK_PATH_SECRET=use_a_different_long_random_secret_for_the_url
MINI_APP_API_ENABLED=true
BOT_OWNER_IDS=1272791365
LOCAL_PERSISTENCE_ENABLED=false
REDIS_LEGACY_PICKLE_LOAD_ENABLED=false
BLOCK_UNSCANNABLE_GENERIC_FILES=true
...
REDIS_URL=redis://...
REDIS_ENABLED=true
REDIS_STATE_SIGNING_SECRET=use_a_long_random_state_signing_secret
...
Local pickle persistence is disabled by default because pickle files can execute code while loading. For production, use signed Redis JSON and/or Supabase JSONB. Only enable `LOCAL_PERSISTENCE_ENABLED=true` for a pickle file created and controlled by your own deployment. Never deploy an uploaded or third-party `.pickle` file.

The webhook URL path secret and Telegram webhook header secret must be different values. The public health endpoint intentionally does not return either secret or the webhook route.

Files with a generic binary MIME type that are too large for content inspection are blocked by default. Set `BLOCK_UNSCANNABLE_GENERIC_FILES=false` only when you accept the renamed-executable risk.
